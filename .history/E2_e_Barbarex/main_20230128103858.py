...
x = dict_produtos_quantidade_janeiro.keys()
y = dict_produtos_quantidade_janeiro.values()


df = pd.DataFrame.from_dict(dict_produtos_quantidade_janeiro, orient='index', columns=['Quantidade']).reset_index().rename(columns={
    'index':'Produto'
})

df = df.sort_values(by='Quantidade', reversed=True)
print(df)
sns.barplot(x='Quantidade', y='Produto', data=df)
plt.show()
