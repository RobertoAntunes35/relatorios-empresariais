...
x = dict_produtos_quantidade_dezembro.keys()
y = dict_produtos_quantidade_dezembro.values()

df = pd.DataFrame.from_dict(dict_produtos_quantidade_dezembro, orient='index', columns=['Quantidade']).reset_index().rename(columns={
    'index':'Produtos'
})

# Gráfico colunas para a quantidade geral de produtos do mês dezembro
df = df.sort_values(by='Quantidade', ascending=False)
print(df)
sns.barplot(x='Quantidade', y='Produtos', data=df)
plt.show()
