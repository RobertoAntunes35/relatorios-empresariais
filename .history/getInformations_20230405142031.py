...
class Clientes(foo.Excel):
    ...
    def clientesEmCadastro(self, codigoVendedor: int) -> pd.DataFrame:
        dados = copy.deepcopy(self.__matrizDados)
        valorFiltro = None
        if codigoVendedor not in self._listaVendedores:
            raise ValueError("O vendedor solicitado para analise, não foi passado como parâmetros para analise.")
        for chave_vendedor, valor_vendedor in vendedores.items():
            if valor_vendedor == codigoVendedor:
                valorFiltro = chave_vendedor
                break
        return dados.loc[dados['nome_vendedor'] == valorFiltro, ['codigo_cliente', 'nome_fantasia', 'cidade', 'dia_visita', 'nome_vendedor']]
...
if __name__ == '__main__':
    ...
    Cliente = Clientes(
        ...
        listaVendedores=[11,12,13,15,16],
        **rename_file_clientes
    )
    print(Cliente.clientesEmCadastro(10))
