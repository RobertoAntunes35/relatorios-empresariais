...
class AnaliseGeralVendedores(foo.Excel):
    ...
    def positivacaoClientePorVendedor(self, clientesVendedor: pd.DataFrame) -> int:
        ...
        print(dados)
        info = set(np.array(dados.loc[dados['nome_vendedor'] == 'DALMO LEVI GOMES NOGUEIRA']['nome_fantasia']))
        print(len(info))
        ...
        for vendedor, nome_vendedor in zip(codigos_vendedores_selecionados, vendedores_selecionados):
            df_positivacao[nome_vendedor] = [len(set(np.array(dados_matriz.loc[dados_matriz['codigo_vendedor'] == vendedor]['nome_fantasia']))),
                len(set(np.array(dados.loc[dados['nome_vendedor'] == nome_vendedor]['nome_fantasia'])))]

        print(len(df_positivacao['DALMO LEVI GOMES NOGUEIRA']))
