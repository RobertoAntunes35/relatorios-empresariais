...
class AnaliseGeralVendedores(foo.Excel):
    ...
    def positivacaoClientePorVendedor(self, clientesVendedor: pd.DataFrame) -> int:
        ...
        for vendedor, nome_vendedor in zip(codigos_vendedores_selecionados, vendedores_selecionados):
            positivacao = (set(np.array(dados_matriz.loc[dados_matriz['codigo_vendedor'] == vendedor]['nome_fantasia']))),
            clientes_em_cadastro = (set(np.array(dados.loc[dados['nome_vendedor'] == nome_vendedor]['nome_fantasia']))),
            porcentagem_positivacao = (positivacao / clientes_em_cadastro)
            df_positivacao[nome_vendedor] = (len(clientes_em_cadastro), len(positivacao), porcentagem_positivacao)
        
        print(df_positivacao)
