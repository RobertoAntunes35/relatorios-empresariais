    ...
    def positivacaoCliente(self, clientesVendedor: pd.DataFrame) -> dict:
        ...
        def clientesporDiaVisita():
            ...
            for nome_vendedor, codigo_vendedor in dados_vendedores.items():
                ...
                clientes_vendedor = set(np.array(dados_matriz[dados_matriz['codigo_vendedor'] == codigo_vendedor]['nome_fantasia']))                
                clientes_dia = {}
                for dia in dias_de_visita:
                    contagem = 0
                    
                    quantidade_clientes_por_dia_visita = (dados.loc[(dados['dia_visita'] == dia) & (dados['nome_vendedor'] == nome_vendedor)]['nome_fantasia']).count()
                    
                    clientes_dia_visita = set(np.array(dados.loc[(dados['dia_visita'] == dia) & (dados['nome_vendedor'] == nome_vendedor)]['nome_fantasia']))
                    ...
                    clientes_dia[dia] = (quantidade_clientes_por_dia_visita, contagem, (contagem/quantidade_clientes_por_dia_visita))

                vendedor_final[codigo_vendedor] = clientes_dia
            print(vendedor_final[10])
