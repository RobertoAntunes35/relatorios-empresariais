...
class Relatorios(FornecedoresFiltro):
    ...
    def quantidadeProdutoPorValor(self, menor_valor: float, maior_valor: float, diferente = None):
        ...
        for clientes_desconto in analise_dados.get('clientes_com_desconto_financeiro'):
            clientes = self.frame_analise.loc[self.frame_analise['nome_fantasia']== clientes_desconto]['valor_venda_fardo']
            
            self.frame_analise.loc[self.frame_analise['nome_fantasia']== clientes_desconto]['valor_venda_fardo'] = clientes*0.97
