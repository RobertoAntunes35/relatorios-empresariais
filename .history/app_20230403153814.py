...
from config import analise_dados, datas, path, FILE_CLIENTES, FILE_PEDIDO_ITENS, FILE_STATIC

from config import vendedores as codigosVendedores
...
class AnaliseGeralVendedores(foo.Excel):
    ...
    def __init__(self, nome_arquivo: str, codigosVendedor: list, mes_analise: str, **columns_select) -> None:
        super().__init__(nome_arquivo, **columns_select)
        
        self._codigoVendedor = codigosVendedor
        self._mesAnalise = mes_analise
        
        if len(self._codigoVendedor) == 0:
            raise ValueError("Atribua ao menos um codigo de vendedor para iniciar a analise") 

        for codVendedor in codigosVendedor:
            if codVendedor not in codigosVendedor:
                raise ValueError("O código %s não está na listagem de vendedores cadastrados." % codVendedor)
            
    @property
    def codigosVendedor(self):
        return self._codigoVendedor
    
    @codigosVendedor.setter
    def codigoVendedor(self, newValue):
        print(type(self._codigoVendedor))
        if newValue not in codigosVendedores.keys():
            raise ValueError("O valor a ser incluso, não é de um fornecedor cadastrado.")
