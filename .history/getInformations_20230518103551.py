...
class AnaliseGeralVendedores(foo.Excel):
            ...
            if self.datas:
                for nome_vendedor, codigo_vendedor in dados_vendedores.items():
                    ...
                    for dia in dias_de_visita:
                        contagem = 0
                        frameDiaVisita = set(np.array(dados.loc[(dados['dia_visita'] == dia) & (dados['nome_vendedor'] == nome_vendedor)]['nome_fantasia']))
    ...
    def diasVisita(self, diasVisita, frame, vendedor):
        for dia in diasVisita:
            contagem = 0
            quantidadeClientesDiaVisita = np.array(frame.loc[(frame['dia_visita']== dia) & frame['nome_vendedor'] == vendedor]['nome_fantasia'])
